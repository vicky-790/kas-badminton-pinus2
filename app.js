...
async function initMembers(){
  ...
  try{
    ...
    renderMembers();
    renderTopTelatReal();         // ✅ refresh top telat setelah members ada
    fillRemoveMemberSelect();     // ✅ isi select hapus anggota
    saveCache();
  }catch(err){
...
/* =========================================================
  FILL SELECT REMOVE MEMBER (ADMIN)
========================================================= */
...
/* =========================================================
  TOP 5 TELAT REAL (jatuh tempo 5/10)
  - kalau hari ini masih < jatuh tempo -> belum dianggap telat
  - kalau hari ini >= jatuh tempo -> yang belum bayar = telat
========================================================= */
function renderTopTelatReal(){
  ...
  // kalau belum lewat jatuh tempo
  if(currentDay < dueDay){
    el.innerHTML = `⏳ Belum dihitung telat. Jatuh tempo tanggal <b>${dueDay}</b>.`;
    return;
  }

  // kumpulkan yang sudah bayar membership bulan ini
  const paidSet = new Set(
    state.payments
      .filter(p => p.jenis === "membership" && p.bulan === bulan && String(p.tahun) === String(tahun))
      .map(p => String(p.nama || "").toLowerCase().trim())
...
// event: ganti jatuh tempo -> rerender
const dueSelect = document.getElementById("dueDaySelect");
if(dueSelect){
...
/* =========================================================
  SUBMIT PEMASUKAN (member manual)
========================================================= */
...
/* =========================================================
  ...
  - update doc settings/members (list array)
========================================================= */
document.getElementById("btnAddMember").addEventListener("click", async ()=>{
  ...
  try{
    ...
    // reload members biar table + dropdown hapus update
    await initMembers();
...
/* =========================================================
  ADMIN: HAPUS ANGGOTA
  - hapus dari list settings/members
========================================================= */
