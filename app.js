...
/* =========================================================
  ✅ PATCH FINAL (KUNCI AMAN)
  - HOME sekarang menampilkan "Saldo Saat Ini" (pemasukan - pengeluaran)
  - BUKAN saldo global dari ledger
  - Layout/UI tidak berubah sama sekali
========================================================= */
function renderLedger(fallback=false){
  ...
  /* =====================================================
    ...
    - saldo home = pemasukan bulan ini - pengeluaran bulan ini
    - sama persis seperti kolom "Saldo Saat Ini" di laporan
  ====================================================== */
  const { bulan, tahun } = state.filter;

  const pemasukan = state.payments
    .filter(p => p.bulan === bulan && String(p.tahun) === String(tahun))
    .reduce((a,b)=> a + Number(b.nominal||0), 0);

  const pengeluaran = state.expenses
    .filter(e => e.bulan === bulan && String(e.tahun) === String(tahun))
    .reduce((a,b)=> a + Number(b.nominal||0), 0);

  const saldoSaatIni = pemasukan - pengeluaran;

  // ✅ tampilkan saldo saat ini di HOME
  saldoEl.textContent = rupiah(saldoSaatIni);

  // ✅ info kecil di bawahnya
  if(updatedEl){
    if(fallback){
      updatedEl.textContent = "Saldo saat ini tidak terbaca (cek rules).";
    }else{
      updatedEl.textContent = `Saldo saat ini bulan ${bulan} ${tahun} (pemasukan - pengeluaran).`;
    }
  }
}
...
function renderDashboard(){
  ...
  // ✅ saldo home (sekarang saldo saat ini)
  renderLedger();
}
